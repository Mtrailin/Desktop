...
class ModelHandler:
    ...
    def load(self, path_or_repo: str) -> None:
        path_or_repo = path_or_repo.strip()
        if not path_or_repo:
            print("[WARN] No model path or repo provided.")
            return None, None, None, None
        ...
        self.model = None  # Fix the invalid syntax here
        model_type = None
        ...
        try:
            ...
            try:
                ...
                # Try causal LM (GPT2/GPT-Neo)
                try:
                    self.model = AutoModelForCausalLM.from_pretrained(path_or_repo)
                    model_type = "causal_lm"
                    print("[INFO] Loaded causal LM model (GPT2/GPT-Neo).")
                except Exception as e:
                    print(f"[ERROR] Failed to load model in any mode: {e}")
        ...
        try:
            if model_type == "classification":
                pipe = pipeline("text-classification", model=self.model, tokenizer=self.tokenizer, device=device)
            elif model_type == "mlm":
                pipe = pipeline("fill-mask", model=self.model, tokenizer=self.tokenizer, device=device)
            elif model_type == "causal_lm":
                pipe = pipeline("text-generation", model=self.model, tokenizer=self.tokenizer, device=device)
        except Exception as e:
            print(f"[ERROR] Failed to create pipeline: {e}")
            pipe = None
    ...
    def infer(
        ...
        top_p: float = 0.9,
        max_length: int = 512
    ) -> Union[str, List[Dict[str, Any]], float]:
...
class ModelHandler:
    def __init__(self, model=None, tokenizer=None, task_type="auto", console=None):
        self.model = model
    ...
    def infer(self, text, output_type="label", temperature=0.7, top_p=1.0, max_length=256):
...
class VedaKynaStudio:
    def __init__(self) -> None:
        ...
        self.var_model_path = tk.StringVar()  # Add missing variable
        self._build_ui()
    ...
    def _build_train_tab(self):
        ...
        # Training UI components will be added here
    ...
    def _build_infer_tab(self):
        ...
        # Inference UI components will be added here

    def _build_hyper_tab(self):
        """Build the hyperlearning tab UI"""
        f = self.tab_hyper
        pad = dict(padx=8, pady=6)

        ttk.Label(f, text="Hyperparameter Learning").pack(pady=10)
        # Hyperlearning UI components will be added here

    def _build_frag_tab(self):
        """Build the fragments tab UI"""
        f = self.tab_frag
        pad = dict(padx=8, pady=6)

        ttk.Label(f, text="Knowledge Fragments").pack(pady=10)
        # Fragment UI components will be added here

    def on_load_model(self):
        """Handle loading model when button is clicked"""
        model_path = self.model_entry.get().strip()
        if not model_path:
            messagebox.showwarning("No Model", "Enter a model path or HuggingFace model name")
            return

        self.btn_load_model.config(state="disabled")
        try:
            self._log(f"[LOAD] Loading model from {model_path}...")
            self.model_handler.load(
                path_or_repo=model_path,
                task_type=self.var_task.get()
            )
            self._log("[LOAD] Model loaded successfully")
            self.cfg.set("model_name", model_path)
            
        except Exception as e:
            self._log(f"[LOAD ERROR] {str(e)}")
            messagebox.showerror("Load Error", str(e))
        finally:
            self.btn_load_model.config(state="normal")

    def _on_article(self, label: str, record: dict):
        """Handle new article data"""
        self.records.append(record)
        # Update UI with new record
        # Will be implemented when article view is added
    ...
    def _wire_autosave(self, var: Any, key: str) -> None:
    ...
    def on_toggle_remember(self) -> None:
